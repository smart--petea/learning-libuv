 ...
 * outputting a string containing the error name (if available in 'ename' array)
 ...
 * function does not flush stdout and calls _exit(2) to terminate the process (rather than exit(3), which would cause
 * exit handlers to be invoked).
 *
 * These differences make this function especially useful in a library
 * function that creates a child process that must then terminate
 * because of an error: the child must terminate without flushing
 * stdio buffers that were parially filled by the caller and without
